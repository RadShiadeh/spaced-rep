...
def update_entry(topic: str, date: str = None, reset_rate: int = 0):
    """
    ...
    try:
        ...
        data_seen[topic][0] = date
        write_data(data_seen, SEEN_PATH)
        
        remove_topic_from_revs(topic, date)
        update_revision(topic, date, reset_rate)
    except Exception:
        raise
    

def remove_topic_from_revs(topic: str, date: str):
    """
...
def add_new_topic(topic: str, date: str, url: Optional[str] = None) -> None:
    """
    ...
    try:
        ...
        update_revision(topic, date)
        write_data(data, SEEN_PATH)
    except Exception as e:
        raise e


def update_revision(topic: str, date: str, reset_idx: int = 0):
    """
    ...
    try:
        ...
        topic = topic.strip().lower()
        data = read_data(REVISIONS_PATH)
        data = build_space_rep(topic, date, reset_idx)
        write_data(data, REVISIONS_PATH)
    except Exception:
        raise
    

def build_space_rep(topic: str, date: str, reset_rate: int = 0):
    """
