...
def update_entry(topic: str, date_to_remove_from: str = None, reset_rate: int = 0):
    ...
    try:
        ...
        print(f"\nupdating the topic: {topic} seen with the new entries: {data_seen[topic]}")
        
        remove_topic_from_revs(topic, date_to_remove_from)
        print(f"\nremoved topic: {topic} from revisions list from {date_to_remove_from} onwards")
        update_revision(topic, date_to_remove_from, reset_rate)
